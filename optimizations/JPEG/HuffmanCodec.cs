...
namespace JPEG
{
    ...
    class HuffmanCodec
    {
        ...
        private static HuffmanNode BuildHuffmanTree(int[] frequences)
        {
            ...
            var countUsedNodes = 0;
            while (countNodes > countUsedNodes + 1)
            {
                var firstMinIndex = 0;
                var secondMinIndex = 0;
                for (var i = 1; i < countNodes; i++)
                    switch (used.Contains(i))
                    {
                        case false when nodes[i].Frequency < nodes[firstMinIndex].Frequency:
                            secondMinIndex = firstMinIndex;
                            firstMinIndex = i;
                            break;
                        case false when nodes[i].Frequency < nodes[secondMinIndex].Frequency:
                            secondMinIndex = i;
                            break;
                    }

                used.Add(firstMinIndex);
                used.Add(secondMinIndex);
                countUsedNodes += 2;
                
                nodes.Add(
                    new HuffmanNode
                    {
                        Frequency = nodes[firstMinIndex].Frequency + nodes[secondMinIndex].Frequency,
                        Left = nodes[secondMinIndex],
                        Right = nodes[firstMinIndex]
                    }
                );
                countNodes++;
            }

            return nodes[countNodes - 1];
        }
